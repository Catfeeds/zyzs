	...
	<form method="POST">
		...
		<div class="x12">
			<div class="form-group padding-large-bottom">
				...
				<h1><span class="icon-edit"> </span>正在为 “<span class="text-main">{{ $navigation->name }}</span>” 增加职位</h1>
				</div>
			</div>
		</div>
		...
		<div class="x12">
			...
			<div class="x7">
				<div class="form-group padding-large-bottom">
				    ...
				    <div class="field"><input type="text" class="input" id="jobname" name="jobname" data-validate="required:必填" value="@if(count(old('jobname'))>0){{ old('jobname') }}@endif" placeholder="职位名称" /></div>
				...
				<div class="form-group padding-large-bottom">
				    ...
				    <div class="field"><input type="number" class="input" id="weihao" name="weihao" data-validate="required:必填,number:只能是数字,compare#>0:值不得小于1,compare#<99999999999:值不得大于99999999999,length#>=1:长度不能小于1个字符,length#<=11:长度不得大于11个字符" value="1500" placeholder="排序" /></div>
				...
				<div class="form-group padding-large-bottom">
				    ...
				    <div class="field"><input type="number" class="input" id="jobcount" name="jobcount" data-validate="required:必填,number:只能是数字,compare#>0:值不得小于1,compare#<99999999999:值不得大于99999999999,length#>=1:长度不能小于1个字符,length#<=11:长度不得大于11个字符" value="@if(count(old('jobcount'))>0){{ old('jobcount')}}@else{{ $defaultint }}@endif" placeholder="初始浏览量" /></div>
				    <div class="text-gray text-small"></div>
				...
				<div class="form-group padding-large-bottom">
				    ...
				    <div class="field"><input type="text" class="input" id="jobplace" name="jobplace" data-validate="required:必填" value="@if(count(old('jobplace'))>0){{ old('jobplace') }}@endif" placeholder="工作地点" /></div>
				    <div class="text-gray text-small"></div>
