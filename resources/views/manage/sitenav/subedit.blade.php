...
<div class="container bg-white margin-large-top">
	<div class="padding-large">
		<form method="POST" action="/manage/sitenav/sub/edit/{{ $editdata->id }}">
		...
		<div class="x12">
			...
			<div class="form-group padding-large-bottom">
				...
				<div class="button-group border-yellow radio pagestylechoose">
					...
					@if($key =="mainmenu" || $key =="menudetails")
					@else
			...
			@if(count(old('sectionid'))>0)
			    <?php $sectionidvalue = old('sectionid');?>
			@else
			    <?php $sectionidvalue = $editdata->sectionid;?>
			@endif
			<div class="form-group padding-large-bottom asidebox @if($sectionidvalue==3)hide @endif" style="width:200px;">
				<label class="label"><strong>侧边栏样式</strong></label>
				<select name="sectionid" class="input">
					@foreach($sections as $section)
			    	<option value="{{$section->id}}" @if($section->id==$sectionidvalue) selected="selected" @endif>{{$section->title}}</option>
			    	@endforeach
			    </select>
			</div>
