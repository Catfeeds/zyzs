...
<div class="container bg-white margin-large-top">
	<div class="padding-large">
		<form method="POST" action="/manage/feedback/ts/{{$feedback->id}}">
		<div class="line-big margin-large-bottom">
			<div class="x6">
				<ul class="bread">
				    <li><a href="/manage/main" class="icon-home"> 管理后台</a> </li>
				    <li><a href="/manage/feedback/list" title=""> 留言管理</a></li>
            <li>推送</li>
				</ul>
			</div>
		...
		<div class="x12">
          ...
          <div class="form-group padding-large-bottom">
              ...
              <div class="field">
                <textarea name="content" cols="50" class="input ckeditor" id="editor1">@if(count(old('content'))>0){!! old('content')!!}@else{!!$feedback->content!!}@endif</textarea>
              </div>
          ...
          <div class="form-group padding-large-bottom">
              ...
              <div class="field">
                <textarea name="reply" cols="50" class="input ckeditor" id="editor2">@if(count(old('reply'))>0){!! old('reply')!!}@else{!!$feedback->reply!!}@endif</textarea>
              </div>
              @if($errors->has('reply'))
          ...
          <div class="form-group padding-large-bottom">
            ...
            <div class="field"><input type="text" class="input" id="keywords" name="keywords" data-validate="required:必填,keywords:错误：只能为中文、英文、数字，用英文小写逗号隔开,length#<=100:长度必须小于100个字符" placeholder="关键词1,关键词2,关键词3" value="@if(count(old('keywords'))>0){{ old('keywords') }}@else{{$feedback->keywords}}@endif" /></div>
          ...
          <div class="form-group padding-large-bottom">
            ...
            <div class="field"><textarea rows="5" class="input" id="description" name="description" placeholder="不超过200个字符，尽量包含关键词" data-validate="required:必填,length#<=800:长度必须小于200个字符">@if(count(old('description'))>0){{ old('description') }}@else{{$feedback->description}}@endif</textarea></div>
          ...
          <div class="form-group padding-large-bottom">
              ...
              <div class="field"><input type="number" class="input" id="y" name="y" data-validate="required:必填" value="@if(count(old('y'))>0){{ old('y') }}@endif" placeholder="支持人数" /></div>
          ...
          <div class="form-group padding-large-bottom">
              ...
              <div class="field"><input type="number" class="input" id="n" name="n" data-validate="required:必填" value="@if(count(old('n'))>0){{ old('n') }}@endif" placeholder="反对人数" /></div>
              <div class="text-gray text-small"></div>
              @if($errors->has('n'))
          ...
          <div class="form-group padding-large-bottom">
            ...
            <div class="field"><input type="number" class="input" id="weihao" name="weihao" data-validate="required:必填,number:只能是数字,compare#>0:值不得小于1,compare#<99999999999:值不得大于99999999999,length#>=1:长度不能小于1个字符,length#<=11:长度不得大于11个字符" value="@if(count(old('weihao'))>0){{ old('weihao')}}@else{{ $feedback->weihao }}@endif" placeholder="排序" /></div>
