...
<div class="container bg-white margin-large-top">
	<div class="padding-large">
		<form method="POST" action="/manage/sitenav/products/{{ $checknav->id }}">
		<div class="line-big margin-large-bottom">
			...
			<div class="x6 text-right">
				...
				<a href="/manage/product/insert/{{ $checknav->id }}" class="button icon-file-o bg-yellow margin-right"> 新增产品</a>
		...
		<div class="x12">
			...
			<span class="icon-thumb-tack"> </span>“{{ $checknav->name }}” 中的所有产品：</h1>

			@if($manageproduct!=="empty")
            ...
            <table class="table table-striped table-bordered">
                <tr><td align="center" width="50">选择</td><td align="center">产品名称</td><td align="center" width="80">日期</td><td align="center" width="80">排序</td><td align="center" width="90">是否显示</td><td align="center" width="70">浏览</td><td align="center" width="70">删除</td><td align="center" width="70">修改</td></tr>
                @if($manageproduct!=="empty")
                @foreach ($manageproduct as $mt)
                <tr>
                <td align="center">
                <input class="icheckbox" name="selectid[]" type="checkbox" value="{{ $mt->id }}">
                </td>
                <td title="产品名称：" content="{{$mt->name}}-{{ $mt->title }}" class="tips" data-toggle="hover" data-place="top">{{ $mt->name }} - {{$mt->title}}<input name="id[]" type="hidden" value="{{ $mt->id }}"></td>
                ...
                <td align="center">@if($mt->showsnot=="1")<a href="/products-view-{{ $mt->id }}.html" class="button button-little bg-blue icon-file-text-o" target="_blank"> 浏览</a>@else<span class="icon-lock text-gray"> 隐藏</span>@endif</td>
                <td align="center"><a href="/manage/product/remove/{{ $mt->id }}" class="button button-little bg-main icon-trash-o" onclick="{if(confirm('删除本条数据后不可恢复，请确认：')){return true;}return false;}"> 删除</a></td>
                <td align="center"><a href="/manage/product/update/{{ $mt->id }}" class="button button-little bg-yellow icon-edit"> 修改</a></td>
                </tr>
                @endforeach
                @else
                <tr><td colspan="8" align="center">没有找到产品，请添加</td></tr>
                @endif
            </table>
            </div>
		</div>
...
{
	$('form').attr('action','/manage/sitenav/productbatchremove');
	$('form').submit();
}
