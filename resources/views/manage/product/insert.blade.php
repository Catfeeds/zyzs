...
<div class="container bg-white margin-large-top">
	...
	<form method="POST" action="/manage/product/insert/{{ $parentid->id }}">
		...
		<div class="x12">
			<div class="form-group padding-large-bottom">
				...
				<h1><span class="icon-edit"> </span>正在为 “<span class="text-main">{{ $parentid->name }}</span>” 增加产品</h1>
				</div>
			</div>
		</div>
		...
		<div class="x12">
			...
			<div class="x7">
				<div class="form-group padding-large-bottom">
				    ...
				    <div class="field"><input type="text" class="input" id="name" name="name" data-validate="required:必填" value="@if(count(old('name'))>0){{ old('name') }}@endif" placeholder="产品基础名称" /></div>
				...
				<div class="form-group padding-large-bottom">
				    ...
				    <div class="field"><input type="text" class="input" id="title" name="title" data-validate="required:必填" value="{{ old('title') }}" placeholder="产品类型（型号/款式）" /></div>
				    <div class="text-gray text-small"></div>
				    @if($errors->has('title'))
		...
		<div class="x12">
			<div class="form-group parameteredit padding-large-bottom">
			    <div class="label"><label for="parameter">产品参数</label></div>
			    <div class="field">
			      <textarea name="parameter" cols="50" class="input ckeditor" id="editor2">{!! old('parameter') !!}</textarea>
			    </div>
