...
<div class="container bg-white margin-large-top">
	...
	<form method="POST" action="/manage/product/update/{{ $editdata->id }}">
		<div class="line-big margin-large-bottom">
			<div class="x10">
				<ul class="bread">
				    <li><a href="/manage/main" class="icon-home"> 管理后台</a> </li>
				    <li><a href="/manage/sitenav">导航管理</a></li>
				    <li><a href="/manage/sitenav/products/{{ $parentid->id }}">{{ $parentid->name }}</a></li>
				    <li>文章管理</li>
				</ul>
			</div>
			<div class="x2 text-right">
				<a href="/manage/sitenav/products/{{ $parentid->id }}" class="button icon-mail-reply bg-black"> 返回</a>
			</div>
		</div>
		...
		<div class="x12">
			<div class="form-group padding-large-bottom">
				...
				<h1><span class="icon-edit"> </span>正在编辑产品 “<span class="text-main">{{ $editdata->name }}</span>”</h1>
				</div>
			</div>
			...
			<div class="x7">
				<div class="form-group padding-large-bottom">
				    <div class="label"><label for="name">产品基础名称</label></div>
				    <div class="field"><input type="text" class="input" id="name" name="name" data-validate="required:必填" value="@if(count(old('name'))>0){{ old('name') }}@else{{$editdata->name}}@endif" placeholder="产品基础名称" /></div>
				...
				<div class="form-group padding-large-bottom">
				    <div class="label"><label for="title">产品类型（型号/款式）</label></div>
				    <div class="field"><input type="text" class="input" id="title" name="title" data-validate="required:必填" value="@if(count(old('title'))>0){{ old('title') }}@else{{$editdata->title}}@endif" placeholder="产品类型（型号/款式）" /></div>
				    <div class="text-gray text-small"></div>
				    @if($errors->has('title'))
				...
				<div class="form-group padding-large-bottom @if($parentid->pagestyle=="2" || $parentid->pagestyle=="3") hide @endif">
				    ...
				    <div class="field clearfix"><div class="x9"><input type="filepath" class="input" id="xFilePath" name="filepath" data-validate="required:必填,img:文件格式不支持" placeholder="预览图" value="@if(count(old('filepath'))>0){{ old('filepath') }}@else{{$editdata->filepath}}@endif" /></div><div class="x2 x1-move text-right"><button type="button" class="button bg-yellow" onclick="BrowseServer( 'Images:/', 'xFilePath' );">选择图片</button></div></div>
			...
			<div class="x12">
				<div class="form-group detailsedit padding-large-bottom">
				    <div class="label"><label for="parameter">产品参数</label></div>
				    <div class="field">
				      <textarea name="parameter" cols="50" class="input ckeditor" id="editor2">@if(count(old('parameter'))>0){!! old('parameter')!!}@else{!!$editdata->parameter!!}@endif</textarea>
				    </div>
				    @if($errors->has('parameter'))
						<div class="text-red icon-exclamation-triangle"> {{$errors->first('parameter') }}</div>
					@endif
				</div>
