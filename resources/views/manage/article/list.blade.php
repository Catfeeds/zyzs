...
<div class="container bg-white margin-large-top">
	<div class="padding-large">
		<form method="POST" action="/manage/sitenav/article/{{ $navigation->id }}">
		<div class="line-big margin-large-bottom">
			...
			<div class="x6 text-right">
				...
				<a href="/manage/article/insert/{{ $navigation->id }}" class="button icon-file-o bg-yellow margin-right"> 新增文章</a>
		...
		<div class="x12">
			...
			<span class="icon-thumb-tack"> </span>“{{ $navigation->name }}” 中的所有文章：</h1>
            ...
            <table class="table table-striped table-bordered">
                <tr><td align="center" width="50">选择</td><td align="center">标题</td><td align="center" width="80">日期</td><td align="center" width="80">排序</td><td align="center" width="90">是否显示</td><td align="center" width="70">浏览</td><td align="center" width="70">删除</td><td align="center" width="70">修改</td></tr>
                @if($articles->count()>0)
                @foreach ($articles as $mt)
                <tr>
                <td align="center">
                <input class="icheckbox" name="selectid[]" type="checkbox" value="{{ $mt->id }}">
                <input type="hidden" name="type" value="{{$mt->type}}">
                </td>
                <td title="标题：" content="{{ $mt->title }}" class="tips" data-toggle="hover" data-place="top">{{ $mt->title }}<input name="id[]" type="hidden" value="{{ $mt->id }}"></td>
                ...
                <td align="center">@if($mt->showsnot=="1")<a href="/article-view-{{ $mt->id }}.html" class="button button-little bg-blue icon-file-text-o" target="_blank"> 浏览</a>@else<span class="icon-lock text-gray"> 隐藏</span>@endif</td>
                <td align="center"><a href="/manage/article/remove/{{ $mt->id }}" class="button button-little bg-main icon-trash-o" onclick="{if(confirm('删除本条数据，相关的评论、阅读量、赞等数据都将一同被删除，不会恢复请确认：')){return true;}return false;}"> 删除</a></td>
                <td align="center"><a href="/manage/article/update/{{ $mt->id }}" class="button button-little bg-yellow icon-edit"> 修改</a></td>
                </tr>
                @endforeach
                @else
                <tr><td colspan="8" align="center">没有找到文章，请添加</td></tr>
                @endif
            </table>
            </div>
		</div>
...
function chooseamenu(ids){
	$('form').attr('action','/manage/sitenav/articlebatchmove');
	$('form').submit();
}

function chooseremove(ids)
{
	$('form').attr('action','/manage/sitenav/articlebatchremove');
	$('form').submit();
}
