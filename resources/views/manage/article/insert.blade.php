...
<div class="container bg-white margin-large-top">
	...
	<form method="POST" action="/manage/aritcle/insert/{{ $parentid->id }}">
		...
		<div class="x12">
			<div class="form-group padding-large-bottom">
				...
				<h1><span class="icon-edit"> </span>正在为 “<span class="text-main">{{ $parentid->name }}</span>” 增加文章</h1>
				</div>
			</div>
		</div>
		...
		<div class="x12">
			...
			<div class="x7">
				<div class="form-group padding-large-bottom">
				    ...
				    <div class="field"><input type="text" class="input" id="title" name="title" data-validate="required:必填" value="{{ old('title') }}" placeholder="文章标题" /></div>
				...
				<div class="form-group padding-large-bottom">
				    ...
				    <div class="field"><input type="text" class="input" id="zz" name="zz" data-validate="required:必填" value="{{ old('zz') }}" placeholder="文章作者" /></div>
				    <div class="text-gray text-small"></div>
				    @if($errors->has('zz'))
				...
				<?php $defaultint=1; ?>
				<div class="form-group padding-large-bottom">
				    <div class="label"><label for="views">初始浏览量</label></div>
				    <div class="field"><input type="number" class="input" id="views" name="views" data-validate="required:必填,number:只能是数字,compare#>0:值不得小于1,compare#<99999999999:值不得大于99999999999,length#>=1:长度不能小于1个字符,length#<=11:长度不得大于11个字符" value="@if(count(old('views'))>0){{ old('views')}}@else{{ $defaultint }}@endif" placeholder="初始浏览量" /></div>
				    <div class="text-gray text-small"></div>
				    @if($errors->has('views'))
				...
				<div class="form-group padding-large-bottom">
				    <div class="label"><label for="praise">初始点赞人数</label></div>
				    <div class="field"><input type="number" class="input" id="praise" name="praise" data-validate="required:必填,number:只能是数字,compare#>0:值不得小于1,compare#<99999999999:值不得大于99999999999,length#>=1:长度不能小于1个字符,length#<=11:长度不得大于11个字符" value="@if(count(old('praise'))>0){{ old('praise')}}@else{{ $defaultint }}@endif" placeholder="初始点赞人数" /></div>
				    <div class="text-gray text-small"></div>
				...
				<div class="form-group padding-large-bottom">
					...
					<div class="field"><input type="text" class="input" id="keywords" name="keywords" data-validate="required:必填,keywords:错误：只能为中文、英文、数字，用英文小写逗号隔开" placeholder="关键词1,关键词2,关键词3" value="{{ old('keywords') }}" /></div>
				...
				<div class="form-group padding-large-bottom">
				    ...
				    <div class="field clearfix"><div class="x9"><input type="filepath" class="input" id="xFilePath" name="filepath" /></div><div class="x2 x1-move text-right"><button type="button" class="button bg-yellow" onclick="BrowseServer( 'Images:/', 'xFilePath' );">选择图片</button></div></div>
