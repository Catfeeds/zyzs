...
<div class="container bg-white margin-large-top">
	...
	<form method="POST" action="/manage/album/update/{{ $editdata->id }}">
		...
		<div class="x12">
			<div class="form-group padding-large-bottom">
				...
				<h1><span class="icon-edit"> </span>正在编辑相册 “<span class="text-main">{{ $editdata->name }}</span>”</h1>
				</div>
			</div>
			...
			<div class="x7">
				<div class="form-group padding-large-bottom">
				    ...
				    <div class="field"><input type="text" class="input" id="name" name="name" data-validate="required:必填" value="@if(count(old('name'))>0){{ old('name') }}@else{{$editdata->name}}@endif" placeholder="相册名称" /></div>
				...
				<div class="form-group padding-large-bottom">
					...
					<div class="field"><input type="text" class="input" id="keywords" name="keywords" data-validate="required:必填,keywords:错误：只能为中文、英文、数字，用英文小写逗号隔开,length#<=100:长度必须小于100个字符" placeholder="关键词1,关键词2,关键词3" value="@if(count(old('keywords'))>0){{ old('keywords') }}@else{{$editdata->keywords}}@endif" /></div>
				...
				<div class="form-group padding-large-bottom">
					...
					<div class="field"><textarea rows="5" class="input fixck" id="description" name="description" placeholder="不超过200个字符，尽量包含关键词" data-validate="required:必填,length#<=800:长度必须小于200个字符">@if(count(old('description'))>0){{ old('description') }}@else{{$editdata->description}}@endif</textarea></div>
				...
				<div class="form-group padding-large-bottom @if($parentid->pagestyle=="2" || $parentid->pagestyle=="3") hide @endif">
				    <div class="label"><label for="cover">预览图</label></div>
				    <div class="field clearfix"><div class="x9"><input type="cover" class="input" id="xFilePath" name="cover" data-validate="required:必填,img:文件格式不支持" placeholder="预览图" value="@if(count(old('cover'))>0){{ old('cover') }}@else{{$editdata->cover}}@endif" /></div><div class="x2 x1-move text-right"><button type="button" class="button bg-yellow" onclick="BrowseServer( 'Images:/', 'xFilePath' );">选择图片</button></div></div>
			...
			<div class="album-photo-box padding-large-top">
				...
				<button type="button" data-id="{{$editdata->id}}" class="button bg-yellow" id="addphotobutton" onclick="BrowseServer( 'Images:/', 'photos' );">增加照片</button>
			</div>
			...
			<div class="form-group padding-large-bottom">
				...
				<?php $countalbum = 0; ?>
				@if(!empty($editdata->options))
				<?php $views = json_decode($editdata->options,true);?>
				@foreach($views as $view)
				<?php $countalbum++; ?>
					<tr id="albums{{$countalbum}}">
						<td><input type="hidden" name="filepath[]" class="input" value="{{$view['filepath']}}"><img src="{{$view['filepath']}}" class="img-responsive"></td>
						<td><input name="title[]" class="input" value="{{$view['title']}}"></td>
						<td><input name="weihaos[]" class="input" value="{{$view['weihao']}}"></td>
						<td><button type="button" id="delete{{$countalbum}}" class="button deletebutton">删除</button></td>
					</tr>
				@endforeach
				@endif
				</table>
...
<script type="text/javascript">
	var countalbum = <?php echo $countalbum+1; ?>;
	function goinsert(){
		countalbum++;
